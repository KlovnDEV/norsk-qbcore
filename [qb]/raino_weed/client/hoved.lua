...
RegisterNetEvent('raino_weed:client:getHousePlants', function(house)
    raino.Functions.TriggerCallback('raino_weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, _ in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    raino.Functions.Progressbar("remove_weed_plant", Lang:t('text.harvesting_plant'), 8000, false, true, {
                                        ...
                                        TriggerServerEvent('raino_weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                                        raino.Functions.Notify("Prosessen kansellert", "error")
                                    end)
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                raino.Functions.Progressbar("remove_weed_plant", Lang:t('text.removing_the_plant'), 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('raino_weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                    raino.Functions.Notify( Lang:t('error.process_canceled'), "error")
                                end)
                            end
                        end
                    end
...
RegisterNetEvent('raino_weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('raino_weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('raino_weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(100, function()
            raino.Functions.TriggerCallback('raino_weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('raino_weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
	...
	LocalPlayer.state:set("inv_busy", true, true)
            raino.Functions.Progressbar("plant_weed_plant", Lang:t('text.planting'), 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
		...
		LocalPlayer.state:set("inv_busy", false, true)
                ...
                ClearPedTasks(ped)
                TriggerServerEvent('raino_weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('raino_weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                raino.Functions.Notify(Lang:t('error.process_canceled'), "error")
		LocalPlayer.state:set("inv_busy", false, true)
            ...
            raino.Functions.Notify(Lang:t('error.cant_place_here'), 'error', 3500)
        end
    else
        raino.Functions.Notify(Lang:t('error.not_safe_here'), 'error', 3500)
    end
end)

RegisterNetEvent('raino_weed:client:foodPlant', function()
    if currentHouse ~= nil then
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    raino.Functions.Notify(Lang:t('error.not_need_nutrition'), 'error', 3500)
                else
		            LocalPlayer.state:set("inv_busy", true, true)
                    raino.Functions.Progressbar("plant_weed_plant", Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent('raino_weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
			            LocalPlayer.state:set("inv_busy", false, true)
                        raino.Functions.Notify(Lang:t('error.process_canceled'), "error")
                ...
                raino.Functions.Notify(Lang:t('error.cant_place_here'), "error")
            end
        else
            raino.Functions.Notify(Lang:t('error.cant_place_here'), "error")
        end
