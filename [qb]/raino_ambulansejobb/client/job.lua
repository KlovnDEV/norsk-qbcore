...
local function GetClosestPlayer()
    local closestPlayers = raino.Functions.GetPlayersFromCoords()
...
function TakeOutVehicle(vehicleInfo)
    ...
    raino.Functions.TriggerCallback('raino:Server:SpawnVehicle', function(netId)
        ...
        if Config.VehicleSettings[vehicleInfo] ~= nil then
            raino.Shared.SetDefaultVehicleExtras(veh, Config.VehicleSettings[vehicleInfo].extras)
        end
...
function MenuGarage()
    ...
    local authorizedVehicles = Config.AuthorizedVehicles[raino.Functions.GetPlayerData().job.grade.level]
    ...
    vehicleMenu[#vehicleMenu + 1] = {
        ...
        txt = "",
        params = {
            event = "raino_menu:client:closeMenu"
        }
...
RegisterNetEvent('raino:Client:OnPlayerLoaded', function()
    ...
    CreateThread(function()
        Wait(1000)
        raino.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('raino:Client:SetDuty', function(duty)
...
function Status()
    if isStatusChecking then
        ...
        exports['raino_menu']:openMenu(statusMenu)
    end
end
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        raino.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] then
                        for _, v2 in pairs(result["WEAPONWOUNDS"]) do
                            TriggerEvent('chat:addMessage', {
                                color = { 255, 0, 0 },
                                multiline = false,
                                args = { Lang:t('info.status'), raino.Shared.Weapons[v2].damagereason }
                            })
                        end
                    elseif result["BLEED"] > 0 then
                        ...
                        raino.Functions.Notify(Lang:t('success.healthy_player'), 'success')
                    end
                end
        ...
        raino.Functions.Notify(Lang:t('error.no_player'), 'error')
    end
end)

RegisterNetEvent('hospital:client:RevivePlayer', function()
    local hasItem = raino.Functions.HasItem('firstaid')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            raino.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 5000, false, true, {
                ...
                raino.Functions.Notify(Lang:t('success.revived'), 'success')
        ...
        raino.Functions.Notify(Lang:t('error.no_firstaid'), "error")
    end
end)

RegisterNetEvent('hospital:client:TreatWounds', function()
    local hasItem = raino.Functions.HasItem('bandage')
    if hasItem then
        local player, distance = GetClosestPlayer()
        if player ~= -1 and distance < 5.0 then
            local playerId = GetPlayerServerId(player)
            raino.Functions.Progressbar("hospital_healwounds", Lang:t('progress.healing'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                raino.Functions.Notify(Lang:t('success.helped_player'), 'success')
        ...
        raino.Functions.Notify(Lang:t('error.no_bandage'), "error")
    end
end)
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "stash" then
                    TriggerEvent('raino_ambulanse:stash')
                elseif variable == "armory" then
                    TriggerEvent('raino_ambulanse:armory')
                elseif variable == "storeheli" then
                    TriggerEvent('raino_ambulanse:storeheli')
                elseif variable == "takeheli" then
                    TriggerEvent('raino_ambulanse:pullheli')
                elseif variable == "roof" then
                    TriggerEvent('raino_ambulanse:elevator_main')
                elseif variable == "main" then
                    TriggerEvent('raino_ambulanse:elevator_roof')
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('raino_ambulanse:stash', function()
    if onDuty then
        TriggerServerEvent("inventory:server:OpenInventory", "stash",
            "ambulancestash_" .. raino.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "ambulancestash_" .. raino.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('raino_ambulanse:armory', function()
    if onDuty then
        ...
        while CheckVehicle do
            if IsControlJustPressed(0, 38) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                    raino.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                else
...
RegisterNetEvent('raino_ambulanse:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('raino_ambulanse:elevator_main', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('EMSToggle:Duty', function()
    onDuty = not onDuty
    TriggerServerEvent("raino:ToggleDuty")
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['raino_core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(k)
            else
                CheckVehicle = false
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['raino_core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(k)
            else
                CheckHeli = false
                exports['raino_core']:HideText()
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['raino_target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                ...
                options = {
                    {
                        type = "client",
                        event = "raino_ambulanse:elevator_roof",
        ...
        for k, v in pairs(Config.Locations["main"]) do
            exports['raino_target']:AddBoxZone("main" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                ...
                options = {
                    {
                        type = "client",
                        event = "raino_ambulanse:elevator_main",
                        icon = "fas fa-hand-point-up",
                        label = "Take Elevator",
                        job = "ambulance"
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    exports['raino_core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls("sign")
                else
                    exports['raino_core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['raino_core']:DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls("stash")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['raino_core']:DrawText(Lang:t('text.armory_button'), 'left')
                    EMSControls("armory")
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['raino_core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls("main")
                else
                    exports['raino_core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                exports['raino_core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['raino_core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls("roof")
                else
                    exports['raino_core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
