...
local function DoLimbAlert()
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            raino.Functions.Notify(limbDamageMsg, "primary")
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        raino.Functions.Notify(Lang:t('info.bleed_alert', {bleedstate = Config.BleedingStates[tonumber(isBleeding)].label}), "error")
    end
end
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(raino.Shared.Weapons) do
        if HasPedBeenDamagedByWeapon(ped, k, 0) then
            detected = true
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords, text)
    ...
    raino.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'ambulance')
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    emsNotified = false
    raino.Functions.Notify(Lang:t('info.healthy'))
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    raino.Functions.Notify(Lang:t('success.wounds_healed'), 'success')
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            raino.Functions.Notify(Lang:t('success.being_helped'), 'success')
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["raino_politi"]:IsHandcuffed() then
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        if raino.Functions.GetPlayerData().charinfo.gender == 1 then
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "beds" then
                    TriggerEvent('raino_ambulanse:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('raino_ambulanse:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        raino.Functions.Progressbar("hospital_checkin", Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            if bedId then
                ...
                raino.Functions.Notify(Lang:t('error.beds_taken'), "error")
            end
        end, function() -- Cancel
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent('raino_ambulanse:beds', function()
