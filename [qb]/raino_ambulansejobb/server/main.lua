...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive)
	...
	Player.Functions.RemoveMoney("bank", Config.BillCost , "respawned-at-hospital")
		exports['raino_funds']:AddMoney("ambulance", Config.BillCost)
	...
	if Player.PlayerData.metadata["injail"] > 0 then
		for k, v in pairs(Config.Locations["jailbeds"]) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('raino:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
					exports['raino_funds']:AddMoney("ambulance", Config.BillCost)
				TriggerClientEvent('hospital:client:SendBillEmail', src, Config.BillCost)
		...
		for k, v in pairs(Config.Locations["beds"]) do
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					Player.Functions.ClearInventory()
					MySQL.update('UPDATE raino SET inventory = ? WHERE citizenid = ?', { json.encode({}), Player.PlayerData.citizenid })
					TriggerClientEvent('raino:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney("bank", Config.BillCost, "respawned-at-hospital")
					exports['raino_funds']:AddMoney("ambulance", Config.BillCost)
...
RegisterNetEvent('hospital:server:ambulanceAlert', function(text)
    ...
    local players = raino.Functions.GetQBPlayers()
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	local Patient = raino.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name =="ambulance" then
			Player.Functions.RemoveItem('bandage', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, raino.Shared.Items['bandage'], "remove")
	...
	if Patient then
		if oldMan then
			if Player.Functions.RemoveMoney("cash", 5000, "revived-player") then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, raino.Shared.Items['firstaid'], "remove")
				TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
			else
				TriggerClientEvent('raino:Notify', src, Lang:t('error.not_enough_money'), "error")
			end
		else
			Player.Functions.RemoveItem('firstaid', 1)
			TriggerClientEvent('inventory:client:ItemBox', src, raino.Shared.Items['firstaid'], "remove")
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    if not doctorCalled then
        ...
        for _, v in pairs(players) do
            if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
                TriggerClientEvent('raino:Notify', v.PlayerData.source, Lang:t('info.dr_needed'), 'ambulance')
            end
        end
        ...
        TriggerClientEvent('raino:Notify', src, 'Legen er allerede blitt varslet', 'error')
    end
end)

RegisterNetEvent('hospital:server:UseFirstAid', function(targetId)
	local src = source
	local Target = raino.Functions.GetPlayer(targetId)
	if Target then
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('raino:Notify', helperId, Lang:t('error.cant_help'), "error")
	end
end)

RegisterNetEvent('hospital:server:removeBandage', function()
	local Player = raino.Functions.GetPlayer(source)
...
raino.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
    local players = raino.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
...
raino.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
	local Player = raino.Functions.GetPlayer(playerId)
	local injuries = {}
...
raino.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
raino.Commands.Add('911e', Lang:t('info.ems_report'), {{name = 'message', help = Lang:t('info.message_sent')}}, false, function(source, args)
	local src = source
	local message
...
raino.Commands.Add("status", Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
		TriggerClientEvent("hospital:client:CheckStatus", src)
	else
		TriggerClientEvent('raino:Notify', src, Lang:t('error.not_ems'), "error")
	end
end)

raino.Commands.Add("heal", Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
...
raino.Commands.Add("revivep", Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == "ambulance" then
...
raino.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = raino.Functions.GetPlayer(tonumber(args[1]))
		if Player then
			TriggerClientEvent('hospital:client:Revive', Player.PlayerData.source)
		else
			TriggerClientEvent('raino:Notify', src, Lang:t('error.not_online'), "error")
		end
	else
...
raino.Commands.Add("setpain", Lang:t('info.pain_level'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = raino.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
raino.Commands.Add("kill", Lang:t('info.kill'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = raino.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
raino.Commands.Add('aheal', Lang:t('info.heal_player_a'), {{name = 'id', help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = raino.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
raino.Functions.CreateUseableItem("ifaks", function(source, item)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
...
raino.Functions.CreateUseableItem("bandage", function(source, item)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
raino.Functions.CreateUseableItem("painkillers", function(source, item)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
raino.Functions.CreateUseableItem("firstaid", function(source, item)
	local src = source
	local Player = raino.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
