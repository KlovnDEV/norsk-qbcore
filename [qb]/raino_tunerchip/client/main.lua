...
RegisterNUICallback('save', function(data, cb)
    raino.Functions.TriggerCallback('raino_tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            raino.Functions.Notify(Lang:t("error.tunerchip_vehicle_tuned"), 'error')

            TriggerServerEvent('raino_tunerchip:server:TuneStatus', raino.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end)
end)

RegisterNetEvent('raino_tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    local closestVehicle = GetClosestVehicle(GetEntityCoords(ped), 5.0, 0, 70)
    local plate = raino.Functions.GetPlate(closestVehicle)
    local vehModel = GetEntityModel(closestVehicle)
    if vehModel ~= 0 then
        raino.Functions.TriggerCallback('raino_tunerchip:server:GetStatus', function(status)
            if status then
                raino.Functions.Notify(Lang:t("success.this_vehicle_has_been_tuned"), 'success')
            else
                raino.Functions.Notify(Lang:t("error.this_vehicle_has_not_been_tuned"), 'error')
            end
        end, plate)
    else
        raino.Functions.Notify(Lang:t("error.no_vehicle_nearby"), 'error')
    end
end)

RegisterNUICallback('checkItem', function(data, cb)
    local retval = false
    local result = raino.Functions.HasItem(data.item)
    if result then
        retval = true
    end
    cb(retval)
...
RegisterNUICallback('reset', function(_, cb)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    raino.Functions.Notify(Lang:t("error.tunerchip_vehicle_has_been_reset"), 'error')
    cb("ok")
end)

RegisterNetEvent('raino_tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        raino.Functions.Progressbar("connect_laptop", Lang:t("error.tunerchip_vehicle_has_been_reset"), 2000, false, true, {
        ...
        raino.Functions.Notify(Lang:t("error.you_are_not_in_a_vehicle"), "error")
    end
end)
