...
raino.Functions.CreateUseableItem("tunerlaptop", function(source)
    TriggerClientEvent('raino_tunerchip:client:openChip', source)
end)

RegisterNetEvent('raino_tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
raino.Functions.CreateCallback('raino_tunerchip:server:HasChip', function(source, cb)
    local src = source
    local Ply = raino.Functions.GetPlayer(src)
...
raino.Functions.CreateCallback('raino_tunerchip:server:GetStatus', function(_, cb, plate)
    cb(tunedVehicles[plate])
end)

raino.Functions.CreateUseableItem("nitrous", function(source)
